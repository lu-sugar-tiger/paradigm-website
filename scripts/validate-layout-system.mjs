...
assert.match(teamwearStory, /padding-inline-start:\s*var\(--teamwear-rail-content-edge\);[\s\S]*?padding-inline-end:\s*calc\(var\(--teamwear-rail-content-edge\) \+ var\(--teamwear-rail-gap\)\);/, "Teamwear rails must start at the content edge and retain one extra trailing gap for complete navigation steps");
assert.match(teamwearStory, /scroll-padding-inline-start:\s*var\(--teamwear-rail-content-edge\);[\s\S]*?scroll-padding-inline-end:\s*calc\(var\(--teamwear-rail-content-edge\) \+ var\(--teamwear-rail-gap\)\);/, "Teamwear rail snap padding must preserve the full final card-plus-gap interval");
assert.match(teamwearStory, /scroll-snap-type:\s*x mandatory;/, "Teamwear rails must use horizontal mandatory snap scrolling");
