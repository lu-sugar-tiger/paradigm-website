...
const MATERIAL_SYMBOLS_STYLESHEET = `https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,400..700,0,-25&icon_names=${[...new Set(Object.values(MATERIAL_ICON_NAMES))].sort().join(",")}&display=block`;
...
export function renderProductCard(product, root = "") {
  ...
  const image = media
    ...
    : "";
  const touchZoom = image ? " data-media-zoom-touch" : "";
...
export function renderResponsiveProductImage({ media, alt, root = "", sizes, loading = "", touchZoom = false }) {
  ...
  return `<img src="${html(resolvedPath(media.src))}"${responsiveAttributes} alt="${html(alt)}" width="${html(width)}" height="${html(height)}"${loadingAttribute}${zoomAttribute}>`;
}
...
export function renderDocument({
  ...
  const baseStyles = ["tokens.css?v=20260831g", "motion.css?v=20260831a", "reset.css?v=20260829a", "base.css", "layout.css", "components.css?v=20260901d", "pages.css?v=20260829a", "color-options.css"];
  ...
  const scriptMarkup = ["app.js?v=20260831a", "search-core.js?v=20260829b", "search.js?v=20260901a", ...dataScripts, "choices.js?v=20260831c", ...interactionScripts].map((file) => `  <script defer src="${html(asset(root, `assets/js/${file}`))}"></script>`).join("\n");
