...
assert.match(searchPage, /assets\/js\/media-zoom\.js\?v=20260831c/, "the Search route must load the cache-busted touch inspection module");
...
assert.match(searchClient, /page:\s*"arrow_forward"/, "every page result must use the Material right-arrow leading icon");
assert.match(searchClient, /external:\s*"arrow_outward"/, "external page results must retain the Material northeast trailing icon");
assert.match(searchClient, /leadingContent\(\s*"search-suggestion__content",\s*element\("span", "search-suggestion__label", label\),\s*RESULT_ICONS\.suggestion\s*\)/, "every suggestion must render its leading icon before the label");
assert.match(searchClient, /leadingContent\(\s*"search-page-result__content",\s*title,\s*RESULT_ICONS\.page\s*\)/, "every internal and external page result must render the same leading right arrow");
assert.match(searchClient, /"material-symbols-outlined material-icon external-link__indicator", RESULT_ICONS\.external/, "external page results must additionally render the shared trailing northeast arrow");
assert.match(searchClient, /icon\.setAttribute\("aria-hidden", "true"\)[\s\S]*?externalIcon\.setAttribute\("aria-hidden", "true"\)/, "decorative Search leading and external trailing icons must remain hidden from assistive technology");
...
assert.match(components, /\.search-suggestion__content,[\s\S]*?\.search-page-result__content\s*\{[^}]*margin-inline-start:\s*calc\(var\(--stroke-width-thin\) \+ var\(--space-5\)\);/, "Search text must begin at the field border plus its Space 5 placeholder inset");
assert.match(components, /\.search-result__leading-indicator\s*\{[^}]*position:\s*absolute;[^}]*inset-block-start:\s*50%;[^}]*inset-inline-end:\s*calc\(100% \+ var\(--external-link-arrow-gap\)\);[^}]*width:\s*1em;[^}]*height:\s*1em;[^}]*translate:\s*0 -50%;[^}]*font-size:\s*var\(--external-link-arrow-size\);/, "Search leading icons must use the existing 1em size and Space 1 absolute distance before aligned text");
assert.match(components, /\.external-link__indicator\s*\{[^}]*inset-inline-start:\s*calc\(100% \+ var\(--external-link-arrow-gap\)\);[^}]*font-size:\s*var\(--external-link-arrow-size\);[^}]*opacity:\s*0;/, "external Search page results must reuse the hidden trailing northeast-arrow geometry");
assert.match(components, /\[data-external-link="true"\]:is\(:hover, :active, :focus-visible\) \.external-link__indicator\s*\{[^}]*opacity:\s*1;[^}]*transform:\s*none;/, "external northeast arrows must reveal on hover, touch press, and keyboard focus");
...
assert.match(components, /\.drawer-nav__child\s*\{[^}]*color:\s*var\(--color-on-background-low\);[^}]*font-size:\s*var\(--type-h6-size\);[^}]*line-height:\s*var\(--type-h6-line-height\)/, "menu subcollections must use H6 and On Background Low");
