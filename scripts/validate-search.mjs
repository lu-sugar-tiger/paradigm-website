...
assert.match(components, /\.search-form\s*\{[\s\S]*?min-height:\s*var\(--control-size-large\)/, "the Search field must use the shared 48px large control token");
assert.match(components, /\.search-form\s*\{[^}]*border:\s*1px solid var\(--color-outline-low\);[^}]*background:\s*var\(--color-background-high\)/, "the Search field must use the one-pixel option-chip border and Background High");
